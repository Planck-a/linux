...
```
查找目录和文件  ------->大范围查找
（1）根据文件名查找  find ./  -name  *.cpp   查找当前目录下所有的cpp文件
（2）根据大小查找   find /etc -size +204800    204800字节块=102400k=100M
（3）根据属性查找    find /etc -cmin 30    30分钟内修改过的文件
（4）与或非    -a 与   -o或
（5）查询结果再处理  find /etc -name init* -a -type f -exec ls -l {}\;   查找目录下名称是以init开头的，只要文件不要目录或者软链接，查找出的内容显示其详细属性信息
```
19、grep sed awk  
```
文本查找------------->小范围查找
